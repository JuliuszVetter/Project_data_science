...
7. Anomaly detection in ECG signals was implemented in a very simple way. It consists of the 'detect_anomalies' function, which calculates the mean value and standard deviation of the signal. The function checks which values of the signal deviate from the mean by more than a specified threshold (in this case, three times the standard deviation). These values are considered anomalies. As before, the next function 'load_detect_and_plot_record' displays the signal and the detected anomalies together on the same plot. Here is an example:
